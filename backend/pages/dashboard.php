...
<body class="d-flex flex-column h-100">
  ...
  <script src="/king/assets/vendor/Chart.js/Chart.min.js"></script>
  <script>
    $(document).ready(function() {
      // ----------------- Tổng số mặt hàng --------------------------
      function getDuLieuBaoCaoTongSoMatHang() {
        $.ajax('/king/backend/api/baocao-tongsomathang.php', {
      ...
      function getDuLieuBaoCaoTongSoKhachHang() {
        $.ajax('/king/backend/api/baocao-tongsokhachhang.php', {
          success: function(data) {
            var dataObj = JSON.parse(data);
            var htmlString = `<h1>${dataObj.SoLuong}</h1>`;
      ...
      function getDuLieuBaoCaoTongSoDonHang() {
        $.ajax('/king/backend/api/baocao-tongsodonhang.php', {
          success: function(data) {
            var dataObj = JSON.parse(data);
            var htmlString = `<h1>${dataObj.SoLuong}</h1>`;
      ...
      function getDuLieuBaoCaoTongSoGopY() {
        $.ajax('/king/backend/api/baocao-tongsogopy.php', {
          success: function(data) {
            var dataObj = JSON.parse(data);
            var htmlString = `<h1>${dataObj.SoLuong}</h1>`;
      ...
      function renderChartThongKeLoaiSanPham() {
        $.ajax({
          url: '/king/backend/api/baocao-thongkeloaisanpham.php',
          type: "GET",
